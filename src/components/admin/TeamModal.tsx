...
export default function TeamModal({
  ...
  useEffect(() => {
    if (member) {
      ...
      // Reset form when no member is selected
      setFormData({
        name: '',
        position: '',
        education: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!formData.name || !formData.position || !formData.education || !formData.year || !formData.course) {
        setError('Please fill in all required fields');
        return;
      }

      const endpoint = member 
        ? `http://localhost:5000/api/team/${member._id}`
      ...
      if (response.data) {
        ...
        setNotification({
          type: 'success',
          message: `Team member ${member ? 'updated' : 'created'} successfully!`
        });
      } else {
        throw new Error(response.data?.error || `Failed to ${member ? 'update' : 'create'} team member`);
      }
    } catch (error: any) {
      console.error('Error saving team member:', error);
      setError(error.response?.data?.error || error.message || `Failed to ${member ? 'update' : 'create'} team member`);
      setNotification({
        type: 'error',
        message: `Failed to ${member ? 'update' : 'create'} team member`
      });
    } finally {
      setIsSaving(false);
    }
